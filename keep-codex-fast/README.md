...
- **No-backup maintain:** only with `--apply --no-backup`; skips metadata backups and restore/manifest artifacts.
...
- archive old chats into Codex's manual-archive layout instead of deleting them
- record chats it archived in `~/.codex/keep-codex-fast-state.json` for later keep-codex-fast-only restores
- normalize older keep-codex-fast archived chats into that manual-archive layout so Codex can show, restore, or delete them from its archived conversation UI
- prune archived thread references from the active sidebar and project-session UI state
...
Apply without backup artifacts only when you explicitly accept harder rollback:

```bash
python scripts/keep_codex_fast.py --apply --no-backup --archive-older-than-days 10 --worktree-older-than-days 7
```

Archive only active sessions from one local calendar date, without running the other maintenance actions:
...
If an apply run was skipped because Codex was still running, the script leaves logs alone and prints a follow-up command. After closing Codex, rotate only the logs with:

```bash
python scripts/keep_codex_fast.py --apply --rotate-logs-only
```

If archived chats still appear in the left sidebar after an older run, repair only the sidebar/project-session references with:

```bash
python scripts/keep_codex_fast.py --apply --prune-sidebar-archived-refs
```

If older keep-codex-fast archived chats do not appear in Codex's archived conversation UI because they were stored under `archived_sessions/keep-codex-fast-*`, repair only that layout with:

```bash
python scripts/keep_codex_fast.py --apply --normalize-archived-layout-only
```

Batch unarchive only chats recorded as archived by keep-codex-fast within the last 7 days:

```bash
python scripts/keep_codex_fast.py --apply --unarchive-sessions
```

Batch unarchive all archived chats, including manual Codex archives:

```bash
python scripts/keep_codex_fast.py --apply --unarchive-sessions --unarchive-scope all
```

Change the time window with `--unarchive-archived-within-days N`, or pass `-1` for no time limit.
...
- older keep-codex-fast archived session layout under `archived_sessions/keep-codex-fast-*`
- keep-codex-fast archived session records in `keep-codex-fast-state.json`
- archived thread references in Codex Desktop sidebar/project-session state
- stale worktrees
