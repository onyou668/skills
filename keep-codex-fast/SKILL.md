...
- Advanced no-backup maintain: `--apply --no-backup`; applies maintenance without metadata backups or restore/manifest artifacts. Use only when the user explicitly accepts harder rollback.
...
Apply defaults to backup-first. If the user explicitly wants no backup artifacts, run:

```bash
python scripts/keep_codex_fast.py --apply --no-backup --archive-older-than-days 10 --worktree-older-than-days 7
```

To archive only active sessions from one local calendar date, without worktree/log/config/metadata/layout maintenance, run:

```bash
python scripts/keep_codex_fast.py --apply --archive-sessions-only --archive-date YYYY-MM-DD
```
...
- Archives old non-pinned sessions to the same flat `~/.codex/archived_sessions/` layout used by Codex's manual archive action, so they can appear in Codex's archived conversation UI and be restored/deleted there.
- Records sessions it archived in `~/.codex/keep-codex-fast-state.json`; this is a private skill-owned state file used for future keep-codex-fast-only restore operations.
- Normalizes older keep-codex-fast archived sessions that were stored under `~/.codex/archived_sessions/keep-codex-fast-*` back into the flat manual-archive layout after backup.
- Prunes archived thread references from Codex Desktop sidebar/project-session state so archived chats stop appearing in the active left sidebar. This includes projectless thread order, workspace-root hints for project chats, pinned-thread references, prompt-history entries keyed by thread id, and per-thread permission state.
...
- Rotates `logs_2.sqlite*` into `~/.codex/archived_logs/` only when above the threshold.
  If Codex still has a log file open, rotation reports the file as in use and skips it; close Codex and rerun apply when you want log rotation.
  If an apply run is skipped because Codex is still running, the script does not rotate logs and prints a follow-up `--rotate-logs-only` command to run after closing Codex.
...
## Sidebar Archived References

Codex Desktop also stores sidebar ordering/grouping in `.codex-global-state.json`. A session can be archived correctly in `state_5.sqlite` while still appearing in the left sidebar if its thread id remains in UI state such as `projectless-thread-ids`, `thread-workspace-root-hints`, `pinned-thread-ids`, `electron-persisted-atom-state.prompt-history`, or `heartbeat-thread-permissions-by-id`.

Normal apply now prunes those sidebar references after the backup is written. To repair only this issue without rotating logs, moving worktrees, or archiving additional sessions, run:

```bash
python scripts/keep_codex_fast.py --apply --prune-sidebar-archived-refs
```

If older keep-codex-fast archived chats do not appear in Codex's archived conversation UI because they were stored under `archived_sessions/keep-codex-fast-*`, repair only that layout without archiving additional sessions:

```bash
python scripts/keep_codex_fast.py --apply --normalize-archived-layout-only
```

To batch unarchive sessions, default to only sessions recorded in `keep-codex-fast-state.json` and archived within the last 7 days:

```bash
python scripts/keep_codex_fast.py --apply --unarchive-sessions
```

To batch unarchive all archived sessions, including manual Codex archives, make the wider scope explicit:

```bash
python scripts/keep_codex_fast.py --apply --unarchive-sessions --unarchive-scope all
```

Use `--unarchive-archived-within-days N` to change the time window, or `--unarchive-archived-within-days -1` for no time limit.

If the goal is only to rotate logs after a skipped/blocked apply, close Codex and run:

```bash
python scripts/keep_codex_fast.py --apply --rotate-logs-only
```

## Log Rotation Only

`logs_2.sqlite*` can be locked while Codex Desktop or a local Codex runtime is running. To rotate only logs after closing Codex, run:

```bash
python scripts/keep_codex_fast.py --apply --rotate-logs-only
```

If a log file is still locked, the script reports `logs_rotate_skipped_in_use` and leaves that file in place.
